...
class TestStudentAuth(unittest.TestCase):
    ...
    def setUp(self):
        ...
        update_password("9999900000", "test@1234", doctype="Student Auth", fieldname="password")
    ...
    def _cleanup(self):
        if frappe.db.exists("Student Auth", "9999900000"):
            frappe.delete_doc("Student Auth", "9999900000", force=True)
            frappe.db.commit()
    ...
    def test_account_locks_after_max_attempts(self):
        ...
        doc = frappe.get_doc("Student Auth", "9999900000")
    ...
    def test_link_student_to_phone(self):
        ...
        doc = frappe.get_doc("Student Auth", "9999900000")
        linked = [r.student for r in doc.students]
    ...
    def test_bulk_create_creates_new_record(self):
        if frappe.db.exists("Student Auth", "8888800000"):
            frappe.delete_doc("Student Auth", "8888800000", force=True)

        bulk_create_auth([
            {"phone": "8888800000", "password": "bulk@pass", "student_id": "ST00000010"}
        ])

        self.assertTrue(frappe.db.exists("Student Auth", "8888800000"))
