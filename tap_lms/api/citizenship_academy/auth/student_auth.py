...
def _require_access_token():
    token = _extract_bearer_token()
    if not token:
        frappe.throw("Missing access token", frappe.AuthenticationError)
    payload = _decode_access_token(token)
    if not payload:
        frappe.throw("Invalid or expired access token", frappe.AuthenticationError)
    return payload
...
def login_with_password(phone=None, password=None):
    ...
    try:
        ...
        remaining = max(0, 5 - doc.failed_attempts)
...
def register_verify_otp(phone=None, otp=None):
    ...
    raw_pass = frappe.cache().get_value(f"pending_reg_raw::{phone}")
    if not raw_pass:
