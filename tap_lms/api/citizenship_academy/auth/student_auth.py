...
def send_register_otp(phone=None):
	...
	frappe.cache().set_value(f"register_otp::{phone}", HARDCODED_OTP, expires_in_sec=OTP_EXPIRY_MINUTES * 60)
	return {"success": True, "otp_sent": True}
...
def verify_register_otp(phone=None, otp=None):
	...
	if not phone or not otp:
		frappe.throw("phone and otp are required", frappe.ValidationError)
	stored = frappe.cache().get_value(f"register_otp::{phone}")
	if not stored:
		return {"success": False, "error": "otp_expired"}
	if stored != otp:
		return {"success": False, "error": "otp_invalid"}
	frappe.cache().delete_value(f"register_otp::{phone}")
