...
def _ensure_student_auth(phone, raw_password):
	"""
	Get or create a Student Auth record for this phone and set its password.

	Key rule: NEVER delete an existing password row. If a row already exists,
	leave it alone — the user set that password and it must be preserved.
	Only write a password row if one does not exist yet.
	"""
	auth_name = frappe.db.get_value("Student Auth", {"phone": phone}, "name")

	if auth_name:
		# Record exists — only set the password if there is no row yet.
		# This handles the edge case where the doc was created but
		# update_password failed partway through on a previous attempt.
		if not _password_exists(auth_name):
			update_password(auth_name, raw_password, doctype="Student Auth", fieldname="password")
			frappe.db.commit()
		return auth_name

	# Brand-new registration — create the doc then set the password.
