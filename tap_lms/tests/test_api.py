...
class MockFrappe:
    def __init__(self):
        ...
        # Configure get_doc and get_all behavior
        self.get_doc = Mock(side_effect=self._mock_get_doc)
        self.get_all = Mock(side_effect=self._mock_get_all)
    
    def _mock_get_doc(self, doctype, name=None, **kwargs):
        """Mock get_doc with realistic behavior"""
        if doctype == "API Key":
            if name == "valid_key":
                return MockFrappeDocument(doctype, name, enabled=1)
            elif name == "disabled_key":
                return MockFrappeDocument(doctype, name, enabled=0)
            else:
                raise self.DoesNotExistError(f"API Key {name} does not exist")
        
        elif doctype == "Student":
            if name == "existing_phone" or name == "existing_student":
                return MockFrappeDocument(doctype, name)
            else:
                raise self.DoesNotExistError(f"Student {name} does not exist")
        
        elif doctype == "Teacher":
            if name == "existing_teacher":
                return MockFrappeDocument(doctype, name)
            else:
                raise self.DoesNotExistError(f"Teacher {name} does not exist")
        
        elif doctype == "OTP Verification":
            if name == "9876543210":
                doc = MockFrappeDocument(doctype, name)
                doc.otp = "1234"
                doc.expiry = datetime.now() + timedelta(minutes=10)
                doc.verified = False
                return doc
            elif name == "expired_phone":
                doc = MockFrappeDocument(doctype, name)
                doc.otp = "1234"
                doc.expiry = datetime.now() - timedelta(minutes=10)
                doc.verified = False
                return doc
            elif name == "verified_phone":
                doc = MockFrappeDocument(doctype, name)
                doc.otp = "1234"
                doc.expiry = datetime.now() + timedelta(minutes=10)
                doc.verified = True
                ...
                raise self.DoesNotExistError(f"OTP for {name} does not exist")
        
        elif doctype == "Batch onboarding":
            if "valid_batch" in str(name):
                return MockFrappeDocument(doctype, name, is_active=1)
            else:
                raise self.DoesNotExistError(f"Batch {name} does not exist")
        
        # Default case
        return MockFrappeDocument(doctype, name, **kwargs)
    
    def _mock_get_all(self, doctype, filters=None, fields=None, **kwargs):
        """Mock get_all with realistic behavior"""
        if doctype == "Student":
            if filters and filters.get('phone') == 'existing_phone':
                return [{'name': 'existing_student'}]
            elif filters and filters.get('glific_id') == 'existing_student':
                return [{'name': 'existing_student'}]
            else:
                return []
            ...
            if filters and filters.get('phone_number') == 'existing_teacher':
                return [{'name': 'existing_teacher'}]
            else:
                return []
        
        elif doctype == "Batch onboarding":
            if filters and 'valid_batch' in str(filters.get('batch_skeyword', '')):
                return [{'name': 'valid_batch_onboarding', 'school': 'SCHOOL_001', 'batch': 'BATCH_001'}]
            else:
                return []
            ...
            return [
                ...
                {'name': 'SCHOOL_002', 'name1': 'Another School', 'keyword': 'another_school'}
            ]
            ...
            return [
                ...
                {'name': 'DIST_002', 'district_name': 'Another District'}
            ]
        
        elif doctype == "City":
            return [
                {'name': 'CITY_001', 'city_name': 'Test City'},
                {'name': 'CITY_002', 'city_name': 'Another City'}
            ]
        
        elif doctype == "Course Verticals":
            return [
                {'name': 'VERT_001', 'name2': 'Math', 'vertical_name': 'Mathematics'},
                {'name': 'VERT_002', 'name2': 'Science', 'vertical_name': 'Science'}
            ]
        
        elif doctype == "Batch":
            return [
                {'name': 'BATCH_001', 'batch_id': 'BATCH_2025_001', 'active': 1},
                {'name': 'BATCH_002', 'batch_id': 'BATCH_2025_002', 'active': 1}
            ]
