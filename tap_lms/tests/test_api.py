...
"""
DIAGNOSTIC TEST SUITE - Find exactly why coverage is stuck at 57%
...
print(f"Available modules: {[m for m in sys.modules.keys() if 'tap' in m or 'frappe' in m]}")

# Minimal mock setup
class MinimalMockFrappe:
    ...
    def __init__(self):
        ...
        self.request.data = '{}'
        self.db = Mock()
        self.db.get_value = Mock(return_value="test")
        self.db.get_all = Mock(return_value=[])
        self.db.sql = Mock(return_value=[])
        self.utils = Mock()
        ...
        self.utils.cint = Mock(return_value=0)
        self.utils.cstr = Mock(return_value="")
        
    def get_doc(self, doctype, filters=None):
        if doctype == "API Key":
            mock_doc = Mock()
...
try:
    import tap_lms.api as api_module
    print(f"SUCCESS: API module imported")
    
    # Get all functions
    api_functions = [name for name in dir(api_module) if callable(getattr(api_module, name)) and not name.startswith('_')]
    print(f"Found {len(api_functions)} functions: {api_functions}")
...
class DiagnosticTests(unittest.TestCase):
    """Diagnostic tests to find coverage issues"""
    ...
    def test_import_verification(self):
        """Verify the API module imported correctly"""
        self.assertTrue(API_IMPORTED, "API module should be imported")
        self.assertIsNotNone(api_module, "API module should not be None")
        self.assertGreater(len(api_functions), 0, "Should have found API functions")
        print(f"DIAGNOSTIC: API functions available: {api_functions}")
    
    @unittest.skipUnless(API_IMPORTED, "API module not available")
    def test_authenticate_api_key_basic(self):
        """Test basic authenticate_api_key function"""
        if not hasattr(api_module, 'authenticate_api_key'):
            self.skipTest("authenticate_api_key not found")
        
        print("DIAGNOSTIC: Testing authenticate_api_key...")
        ...
        # Test 2: Invalid key with exception
        try:
            with patch.object(mock_frappe, 'get_doc', side_effect=mock_frappe.DoesNotExistError("Not found")):
    ...
    @unittest.skipUnless(API_IMPORTED, "API module not available")
    def test_list_districts_basic(self):
        """Test basic list_districts function"""
        if not hasattr(api_module, 'list_districts'):
            self.skipTest("list_districts not found")
        
        print("DIAGNOSTIC: Testing list_districts...")
        ...
        try:
            ...
            print(f"list_districts result: {result}")
        except Exception as e:
            print(f"list_districts error: {e}")
            import traceback
            traceback.print_exc()
    
    @unittest.skipUnless(API_IMPORTED, "API module not available")
    def test_send_whatsapp_message_basic(self):
        """Test basic send_whatsapp_message function"""
        if not hasattr(api_module, 'send_whatsapp_message'):
            self.skipTest("send_whatsapp_message not found")
        
        print("DIAGNOSTIC: Testing send_whatsapp_message...")
    ...
    def test_create_student_basic(self):
        ...
        try:
            ...
            print(f"create_student result: {result}")
        except Exception as e:
    ...
    def test_coverage_specific_branches(self):
        """Test specific branches that commonly cause low coverage"""
        print("DIAGNOSTIC: Testing specific problematic branches...")
        
        if not API_IMPORTED:
            self.skipTest("API module not available")
        
        # Test 1: Exception handling branches
        for func_name in api_functions:
            ...
            print(f"Testing exception handling for: {func_name}")
            ...
            try:
                with patch.object(mock_frappe, 'get_doc', side_effect=Exception("DB Error")):
                    ...
                    elif func_name in ['create_student', 'create_teacher']:
                        result = func()
                    elif func_name == 'send_whatsapp_message':
                        result = func('9876543210', 'test')
                    else:
                        result = func()
                print(f"{func_name} exception handling: OK")
            except Exception as e:
                print(f"{func_name} exception handling error: {e}")
        
        # Test 2: HTTP status code branches
        http_functions = ['list_districts', 'list_cities', 'verify_keyword', 'list_schools']
        for func_name in http_functions:
            if hasattr(api_module, func_name):
                print(f"Testing HTTP branches for: {func_name}")
                ...
                try:
                    result = func()
                    print(f"{func_name} invalid key: {mock_frappe.response.http_status_code}")
                except Exception as e:
                    print(f"{func_name} invalid key error: {e}")
                
                # Test missing data (400)
                mock_frappe.request.data = json.dumps({})
                try:
                    result = func()
                    print(f"{func_name} missing data: {mock_frappe.response.http_status_code}")
                except Exception as e:
                    print(f"{func_name} missing data error: {e}")
    
    def test_manual_line_coverage(self):
        """Manually test specific lines that are commonly missed"""
        print("DIAGNOSTIC: Testing specific uncovered lines...")
        
        if not API_IMPORTED:
            self.skipTest("API module not available")
        
        # Test helper functions that might not be called
        helper_functions = [
        ...
        for helper_name in helper_functions:
            if hasattr(api_module, helper_name):
                helper_func = getattr(api_module, helper_name)
                print(f"Testing helper: {helper_name}")
                
                try:
                    if helper_name == 'get_tap_language':
                        result = helper_func('English')
                    elif helper_name == 'determine_student_type':
                        result = helper_func('9876543210', 'John Doe', 'VERTICAL_001')
                    elif helper_name == 'get_current_academic_year':
                        result = helper_func()
                    elif helper_name == 'create_new_student':
                        result = helper_func('John', '9876543210', 'Male', 'SCHOOL_001', '5', 'English', 'glific_123')
                    else:
                        # Try calling with reasonable defaults
                        result = helper_func()
                    print(f"{helper_name}: Called successfully")
                except Exception as e:
                    print(f"{helper_name} error: {e}")
