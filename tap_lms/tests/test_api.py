...
class SimplifiedMockFrappe:
    def __init__(self):
        ...
        self.utils.getdate = lambda x=None: datetime.now().date() if x is None else datetime.strptime(x, '%Y-%m-%d').date()
        self.utils.cstr = lambda x: str(x) if x is not None else ""
        self.utils.get_datetime = lambda x: datetime.strptime(x, '%Y-%m-%d %H:%M:%S') if isinstance(x, str) else x
    ...
    def _setup_core_mocks(self):
        def mock_get_doc(doctype, filters=None, **kwargs):
            if doctype == "API Key" and filters and filters.get('key') == 'valid_key':
                doc = Mock()
                doc.name = 'valid_key'
                doc.enabled = 1
                return doc
...
# Setup mocks
mock_frappe = SimplifiedMockFrappe()
mock_requests = Mock()
mock_glific = Mock()
mock_background = Mock()
...
try:
    ...
    print(f"Could not import API: {e}")
    API_IMPORTED = False
    api_module = None
...
if __name__ == '__main__':
    # Only run the targeted test class
    suite = unittest.TestLoader().loadTestsFromTestCase(TestAPIFor80PercentCoverage)
