...
"""
Additional test methods to achieve 100% coverage for tap_lms/api.py
Add these methods to your existing TestComplete100CoverageAPI class
"""
...
@unittest.skipUnless(API_MODULE_IMPORTED, "API module not available")
def test_logging_and_error_handling_branches(self):
    """Test logging and error handling branches"""
    ...
    functions_with_logging = [
        ...
        'get_course_level', 'send_otp', 'verify_otp', 'update_teacher_role',
    ...
    for func_name in functions_with_logging:
        ...
        if func_name in ['list_districts', 'list_cities']:
            ...
            mock_frappe.request.data = json.dumps({'api_key': 'valid_key', 'glific_id': 'test_glific'})
        elif func_name in ['get_school_city', 'search_schools_by_city']:
        ...
        # Force exception to test error logging paths
        with patch.object(mock_frappe, 'get_all', side_effect=Exception("Forced error for logging test")):
            result = safe_call_function(func)
        
        # Test frappe.log_error itself failing
        with patch.object(mock_frappe, 'log_error', side_effect=Exception("Log error failed")):
            with patch.object(mock_frappe, 'get_all', side_effect=Exception("Original error")):
                result = safe_call_function(func)

# Add this final test to ensure all remaining edge cases are covered
@unittest.skipUnless(API_MODULE_IMPORTED, "API module not available")
