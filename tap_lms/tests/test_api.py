...
"""
Fixed test suite for tap_lms/api.py
This version addresses common testing issues and provides proper mocking
"""
...
# =============================================================================
# MOCK FRAPPE SETUP - Enhanced Version
# =============================================================================

class MockFrappe:
    """Enhanced Mock Frappe module for testing"""
...
# =============================================================================
# TEST CLASSES FOR ACTUAL API FUNCTIONS - FIXED VERSION
# =============================================================================
...
class TestAuthenticationAPI(unittest.TestCase):
    ...
    @patch('frappe.get_doc')  # Patch at module level
    def test_authenticate_api_key_valid(self, mock_get_doc):
        """Test authenticate_api_key with valid key"""
        mock_doc = Mock()
        mock_doc.name = "valid_key"
...
class TestLocationAPI(unittest.TestCase):
    ...
    @patch('tap_lms.api.authenticate_api_key')
    @patch('frappe.get_all')  # Patch at module level
    def test_list_districts_success(self, mock_get_all, mock_auth):
        """Test list_districts with valid input"""
        mock_auth.return_value = "valid_key"
        mock_get_all.return_value = [
        ...
        # Ensure proper request data setup
        mock_frappe.request.data = json.dumps({
            'api_key': 'valid_key',
            'state': 'test_state'
        })
...
class TestOTPAPI(unittest.TestCase):
    ...
    @patch('tap_lms.api.authenticate_api_key')
    @patch('frappe.get_all')  # Patch at module level
    @patch('frappe.db.get_value')  # Patch at module level
    @patch('tap_lms.api.get_active_batch_for_school')
    def test_send_otp_existing_teacher(self, mock_get_batch, mock_get_value, mock_get_all, mock_auth):
        """Test send_otp for existing teacher"""
        mock_auth.return_value = "valid_key"
        mock_get_all.return_value = [{"name": "TEACHER_001", "school_id": "SCHOOL_001"}]
...
class TestStudentAPI(unittest.TestCase):
    ...
    def test_create_student_success(self, mock_create, mock_get_course, mock_get_doc, mock_get_all, mock_auth):
        """Test successful student creation"""
        mock_auth.return_value = "valid_key"
...
class TestWhatsAppIntegration(unittest.TestCase):
    ...
    @patch('frappe.get_single')  # Patch at module level
    @patch('requests.post')
    def test_send_whatsapp_message_success(self, mock_post, mock_get_single):
        """Test successful WhatsApp message sending"""
        # Mock Gupshup settings
...
class TestHelperFunctions(unittest.TestCase):
    ...
    @patch('frappe.db.sql')  # Patch at module level
    def test_determine_student_type_new(self, mock_sql):
        """Test determine_student_type for new student"""
        mock_sql.return_value = []  # No existing enrollment
    ...
    @patch('frappe.utils.getdate')  # Patch at module level
    def test_get_current_academic_year(self, mock_getdate):
        """Test get_current_academic_year function"""
        # Test for April (new academic year)
        mock_getdate.return_value = datetime(2025, 4, 15).date()
    ...
    @patch('frappe.get_all')  # Patch at module level
    @patch('frappe.db.get_value')  # Patch at module level
    def test_get_active_batch_for_school_found(self, mock_get_value, mock_get_all):
        """Test get_active_batch_for_school when batch is found"""
        mock_get_all.side_effect = [
            [{"name": "BATCH_001"}],  # Active batches - ensure proper structure
            [{"batch": "BATCH_001"}]  # Active batch onboardings
        ]
...
def run_specific_test(test_class_name, test_method_name):
    """Helper function to run a specific test for debugging"""
...
def debug_mock_calls(mock_obj, description=""):
    """Helper function to debug mock calls"""
    print(f"\n=== Debug {description} ===")
    print(f"Called: {mock_obj.called}")
    print(f"Call count: {mock_obj.call_count}")
    print(f"Call args: {mock_obj.call_args}")
    print(f"Call args list: {mock_obj.call_args_list}")
    print("=" * 50)
