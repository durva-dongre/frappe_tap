"""
Test cases for StageFlow doctype to achieve 100% coverage
"""
...
def test_pass_statement_coverage(mock_frappe):
    ...
    with patch.dict('sys.modules', mock_frappe):
        from tap_lms.tap_lms.doctype.stageflow.stageflow import StageFlow
...
def test_instance_creation_with_data(mock_frappe):
    """Test creating StageFlow instance with data"""
    with patch.dict('sys.modules', mock_frappe):
        from tap_lms.tap_lms.doctype.stageflow.stageflow import StageFlow
        
        # Create instance with sample workflow data
        data = {
            'doctype': 'StageFlow',
            'name': 'Test Workflow',
