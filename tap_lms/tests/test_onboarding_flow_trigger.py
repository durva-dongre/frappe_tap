...
class TestOnboardingFlowFunctions(unittest.TestCase):
    ...
    # Test trigger_onboarding_flow main function
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.onboarding_flow_trigger.frappe')
    def test_trigger_onboarding_flow_missing_inputs(self, mock_frappe):
        """Test trigger_onboarding_flow with missing inputs"""
        mock_frappe.throw = Mock(side_effect=Exception("Both Backend Student Onboarding Set and Onboarding Stage are required"))
        mock_frappe.whitelist.return_value = lambda f: f
        
        with self.assertRaises(Exception):
            self.trigger_onboarding_flow(None, None)

    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.onboarding_flow_trigger.frappe')
    def test_trigger_onboarding_flow_missing_student_status(self, mock_frappe):
        """Test trigger_onboarding_flow with missing student status"""
        mock_frappe.throw = Mock(side_effect=Exception("Student status is required"))
        mock_frappe.whitelist.return_value = lambda f: f
        
        with self.assertRaises(Exception):
            self.trigger_onboarding_flow("SET_001", "STAGE_001", None)
    ...
    def test_trigger_onboarding_flow_inactive_stage(self, mock_frappe):
        ...
        mock_frappe.throw = Mock(side_effect=Exception("Selected Onboarding Stage is not active"))
        
        with self.assertRaises(Exception):
            self.trigger_onboarding_flow("SET_001", "STAGE_001", "not_started")
    ...
    def test_trigger_onboarding_flow_unprocessed_onboarding(self, mock_frappe):
        ...
        mock_frappe.throw = Mock(side_effect=Exception("Selected Backend Student Onboarding Set is not in Processed status"))
        
        with self.assertRaises(Exception):
            self.trigger_onboarding_flow("SET_001", "STAGE_001", "not_started")

    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.onboarding_flow_trigger.frappe')
    def test_trigger_onboarding_flow_with_stage_flows(self, mock_frappe):
        ...
        mock_stage.stage_flows = [
            ...
            MagicMock(student_status="completed", glific_flow_id="flow_456", flow_type="Personal")
        ]
        
        mock_onboarding = MagicMock()
        mock_onboarding.status = "Processed"
        mock_onboarding.name = "SET_001"
        
        mock_frappe.get_doc.side_effect = [mock_stage, mock_onboarding]
        mock_frappe.enqueue.return_value = "job_123"
        mock_frappe.logger.return_value = MagicMock()
        
        result = self.trigger_onboarding_flow("SET_001", "STAGE_001", "not_started")
    ...
    def test_trigger_onboarding_flow_no_matching_flow(self, mock_frappe):
        ...
        mock_frappe.get_doc.side_effect = [mock_stage, mock_onboarding]
        mock_frappe.throw = Mock(side_effect=Exception("No flow configured"))
        
        with self.assertRaises(Exception):
            self.trigger_onboarding_flow("SET_001", "STAGE_001", "not_started")

    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.onboarding_flow_trigger.frappe')
    def test_trigger_onboarding_flow_legacy_fields(self, mock_frappe):
        ...
        self.assertTrue(result["success"])
        mock_frappe.logger().warning.assert_called()  # Should log deprecation warning
    ...
    def test_trigger_onboarding_flow_job_group_flow(self, mock_auth, mock_frappe):
        ...
        with patch.object(self, 'trigger_group_flow', return_value={"group_count": 10}):
            result = self._trigger_onboarding_flow_job("set", "stage", "status", "flow", "Group")
            self.assertEqual(result["group_count"], 10)

    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.onboarding_flow_trigger.frappe')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.onboarding_flow_trigger.get_glific_auth_headers')
    def test_trigger_onboarding_flow_job_personal_flow(self, mock_auth, mock_frappe):
        """Test _trigger_onboarding_flow_job with personal flow"""
        mock_auth.return_value = {"authorization": "Bearer token"}
        ...
        with patch.object(self, 'trigger_individual_flows', return_value={"individual_count": 5}):
            result = self._trigger_onboarding_flow_job("set", "stage", "status", "flow", "Personal")
    ...
    def test_get_stage_flow_statuses_with_flows(self, mock_frappe):
        ...
        self.assertEqual(len(set(result["statuses"])), 2)  # Should deduplicate

    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.onboarding_flow_trigger.frappe')
    def test_get_stage_flow_statuses_legacy(self, mock_frappe):
    ...
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.onboarding_flow_trigger.frappe')
    def test_get_stage_flow_statuses_error(self, mock_frappe):
        """Test get_stage_flow_statuses with error"""
        mock_frappe.get_doc.side_effect = Exception("Stage not found")
    ...
    # Test get_students_from_onboarding with filters
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.onboarding_flow_trigger.frappe')
    def test_get_students_from_onboarding_with_stage_filter(self, mock_frappe):
        """Test get_students_from_onboarding with stage filter only"""
        mock_onboarding = MagicMock()
        mock_onboarding.name = "SET_001"
        ...
        mock_frappe.get_all.side_effect = [
            ...
            [{"name": "PROGRESS_001"}],  # Stage progress for STUD_001
            []  # No stage progress for STUD_002
        ]
        
        mock_student1 = MagicMock(name="STUD_001")
        mock_student2 = MagicMock(name="STUD_002")
    ...
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.onboarding_flow_trigger.frappe')
    def test_get_students_from_onboarding_not_started_special_case(self, mock_frappe):
        ...
        # Mock calls for regular check and not_started special case
        mock_frappe.get_all.side_effect = [
            backend_students,  # Backend students
            [],  # No stage progress for STUD_001 (regular check)
            [],  # No stage progress for STUD_002 (regular check)
            backend_students,  # Backend students again for not_started check
            [],  # No stage progress for STUD_001 (not_started check)
            [{"name": "PROGRESS_001"}]  # Has stage progress for STUD_002
        ]
    ...
    # Test update_student_stage_progress variations
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.onboarding_flow_trigger.frappe')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.onboarding_flow_trigger.now_datetime')
    def test_update_student_stage_progress_update_existing(self, mock_now, mock_frappe):
        ...
        mock_frappe.get_all.return_value = [{"name": "EXISTING_001"}]
