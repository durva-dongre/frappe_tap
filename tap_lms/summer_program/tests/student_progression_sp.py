...
def _complete_quiz_sp(attempt, quiz_doc, questions, language=None):
    ...
    # The on_update hook (quiz_points.handle_attempt_update) just fired
    # inside attempt.save() and wrote points_earned via frappe.db.set_value.
    # That bypasses the in-memory doc, so reload to pick up the new value
    # for the response below (where quiz_score = gamification points earned).
    attempt.reload()
    ...
    response = {
        ...
        # percentage is still available as `quiz_score_percentage` for
        # any flow that needs it.
