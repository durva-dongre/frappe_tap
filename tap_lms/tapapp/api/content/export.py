...
def _lang_name(lang_code):
    # Used for translation child tables (VideoTranslation, QuizTranslation, etc) where
    # English content lives directly on the parent doc, so "en" deliberately resolves
    # to None -> callers fall back to the untranslated field.
...
def _lang_name_strict(lang_code):
    # Same lookup as _lang_name but without the "en" shortcut. Needed anywhere a
    # language is a required field on the row itself (e.g. Assignment Submission Rule),
    # rather than an optional translation, since English rows still need to be matched
    # by their actual TAP Language name.
    if not lang_code:
        return None
    row = frappe.db.sql(
        'SELECT name FROM "tabTAP Language" WHERE language_code = %s LIMIT 1',
...
def _build_assignment(assignment_name, lang_name_strict, counters):
    # Pulls the assignment name/description/guideline text, translated where available,
    # plus its submission rules. Submission rules are a bit different from the other
    # translation tables in this file: each rule row already belongs to one specific
    # language (it's a required field on the child table), so instead of falling back
    # from a translation row we just filter the rules down to whichever language is
    # currently being exported.
    if not assignment_name:
        return None

    row = frappe.db.sql(
        'SELECT name, assignment_name, description, submission_guidelines'
    ...
    guidelines = _strip_html(a.submission_guidelines)

    if lang_name_strict:
        tr = frappe.db.sql(
            'SELECT translated_title, translated_description, translated_guidelines'
            ' FROM "tabAssignmentTranslation" WHERE parent = %s AND language = %s LIMIT 1',
            (assignment_name, lang_name_strict),
            as_dict=True,
        )
        if tr:
            t = tr[0]
            ...
            desc = _strip_html(t.translated_description) or desc
            guidelines = _strip_html(t.translated_guidelines) or guidelines

    rules = []
    if lang_name_strict:
        # Only pull the submission rule rows that match the language being exported,
        # in the order the editor set for them (display_order), each split cleanly
        # into its guided and unguided text.
        rule_rows = frappe.db.sql(
            'SELECT submission_label, guided_text, unguided_text'
            ' FROM "tabAssignment Submission Rule"'
            ' WHERE parent = %s AND language = %s'
            ' ORDER BY display_order ASC, idx ASC',
            (assignment_name, lang_name_strict),
            as_dict=True,
        )
        for r in rule_rows:
            rules.append(_c({
                ...
                "guided": _strip_html(r.guided_text),
                "unguided": _strip_html(r.unguided_text),
            }))
...
def _build_unit(lu_name, lang_name, lang_name_strict, counters):
    row = frappe.db.sql(
    ...
    for ci in content_items:
        ...
        if ct == "videoclass":
            ...
            aobj = _build_assignment(content_ref, lang_name_strict, counters)
            if aobj:
...
def _build_course(cl_name, lang_name, lang_name_strict, counters):
    row = frappe.db.sql(
    ...
    units = []
    for r in lu_rows:
        u = _build_unit(r.learning_unit, lang_name, lang_name_strict, counters)
        if u:
            units.append(u)
...
def export_program_content(program_id=None, include_r2=False, langs=None):
    ...
    # Two caches, because "en" needs different handling depending on the table:
    # translation tables treat English as the untranslated default (lang_name -> None),
    # while submission rules require a real language name to filter on even for English.
    lang_name_cache = {lc: _lang_name(lc) for lc in lang_list}
    lang_name_strict_cache = {lc: _lang_name_strict(lc) for lc in lang_list}
    ...
    for lang in lang_list:
        ...
        ln_strict = lang_name_strict_cache[lang]
        ...
        for cid in course_ids:
            data = _build_course(cid, ln, ln_strict, counters)
            if data:
                courses[cid] = data
